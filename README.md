...
- **极致压缩 (ZSTD:9)**：强制开启 Btrfs **ZSTD:9** 高等级透明压缩。1G 磁盘空间通过高倍率压缩，可容纳远超物理容量的软件规模。
...
1. **密钥对**：选择您现有的 SSH 密钥对（推荐）。
2. **自定义密码**：手动输入您要设置的 root 密码。

> **⚠️ 注意：** 因安全原因使用 root 的情况下，**默认登录名：`debian`**。

![阿里云登录凭证设置](https://i0.du0.org/i/11f0/169eb452b6e5df.webp)
...
使用 `debian` 用户登录，执行 `sudo -i` 命令即可获得 root 权限。
