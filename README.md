...
### 2. 磁盘扩容说明
本镜像支持两种方式识别阿里云控制台新增的磁盘空间：

- **自动扩容（推荐）**：在控制台调大磁盘后，直接在系统内执行 `reboot`。`Cloud-init` 会在开机阶段自动识别并完成分区表拉伸与文件系统扩容，实现零操作同步。
- **手动在线扩容（无需重启）**：若不方便重启，请先通过 `lsblk` 或 `df -h /` 确认根目录 `/` 所在的分区编号，然后执行：

  ```bash
  # 1. 扩展底层分区
  # 注意：BIOS 引导镜像通常为 1，UEFI 引导镜像通常为 3
  growpart /dev/vda <分区编号>

  # 2. 告知 Btrfs 文件系统占满整个新分区
